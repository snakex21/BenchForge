...
function createProviderConfigsTable(database) {
  database.exec(`
  ...
  const columns = database.prepare(`PRAGMA table_info(provider_configs)`).all().map((c) => c.name)
  if (!columns.includes('display_name')) {
    database.exec(`ALTER TABLE provider_configs ADD COLUMN display_name TEXT;`)
  }
  if (!columns.includes('icon_url')) {
    database.exec(`ALTER TABLE provider_configs ADD COLUMN icon_url TEXT;`)
  }
  if (!columns.includes('is_custom')) {
    database.exec(`ALTER TABLE provider_configs ADD COLUMN is_custom INTEGER DEFAULT 0;`)
  }
}
...
function ensureModelsSchema(database) {
  ...
  if (expected.every((column) => columns.includes(column))) {
    ...
    // Add active column for model toggle (v1.1 migration)
    if (!columns.includes('active')) {
      database.exec(`ALTER TABLE models ADD COLUMN active INTEGER DEFAULT 1;`)
    }
    return
  }
...
function clearDiscoveredModelsForProvider(provider) {
  getDb().prepare(`DELETE FROM discovered_models WHERE provider = ?`).run(provider)
}

const MODEL_SELECT = `id, name, mode, provider, base_url, api_key, model_id, active, input_price_per_1m, output_price_per_1m, pricing_source, pricing_model_id, pricing_updated_at, created_at`
...
// Returns model with API key resolved from provider_configs if model has no own key
function getEffectiveModelConfig(modelId) {
  const model = getModelById(modelId)
  if (!model) return null

  // If model already has its own API key, use it as-is
  if (model.api_key && model.api_key.trim()) return model

  // Try to inherit from provider config
  if (model.provider && model.mode === 'api') {
    const providerConfig = getProviderConfig(model.provider)
    if (providerConfig?.api_key) {
      return { ...model, api_key: providerConfig.api_key }
    }
    if (providerConfig?.base_url && !model.base_url) {
      return { ...model, base_url: providerConfig.base_url }
    }
  }
...
function addModel(data) {
  ...
  const result = getDb().prepare(`
    INSERT INTO models (name, mode, provider, base_url, api_key, model_id, active, input_price_per_1m, output_price_per_1m, pricing_source, pricing_model_id, pricing_updated_at)
    VALUES (@name, @mode, @provider, @base_url, @api_key, @model_id, @active, @input_price_per_1m, @output_price_per_1m, @pricing_source, @pricing_model_id, @pricing_updated_at)
  `).run({
    ...
    active: normalized.active == null ? 1 : (normalized.active ? 1 : 0),
...
function deleteModel(id) {
  ...
  // Clear judge_model_id preference if it references the deleted model
  const judgeModelId = getPreference('judge_model_id')
  if (judgeModelId === id) {
    savePreference('judge_model_id', null)
  }

  // Clean orphan run_sessions referencing the deleted model
  db.prepare(`DELETE FROM run_sessions WHERE model_id = ?`).run(id)
...
// ---- Provider configs (per-provider API key + base_url) ----

function getAllProviderConfigs() {
  const rows = getDb().prepare(`SELECT * FROM provider_configs`).all()
  return rows.map((row) => ({
    provider: row.provider,
    api_key: decryptSecret(row.api_key),
    base_url: row.base_url,
...
function getProviderConfig(provider) {
  const row = getDb().prepare(`SELECT * FROM provider_configs WHERE provider = ?`).get(provider)
  if (!row) return null
  return {
    provider: row.provider,
    api_key: decryptSecret(row.api_key),
...
function saveProviderConfig(provider, data) {
  const db = getDb()
  const encryptedKey = data.api_key != null ? encryptSecret(data.api_key) : null
  db.prepare(`
    INSERT INTO provider_configs (provider, api_key, base_url, display_name, icon_url, is_custom, updated_at)
    VALUES (@provider, @api_key, @base_url, @display_name, @icon_url, @is_custom, datetime('now'))
...
function deleteProviderConfig(provider) {
  return getDb().prepare(`DELETE FROM provider_configs WHERE provider = ?`).run(provider)
}
...
module.exports = {
  ...
  clearDiscoveredModelsForProvider,
  getBenchmarks,
  getBenchmarkById,
  getTasks,
  getAllTasks,
