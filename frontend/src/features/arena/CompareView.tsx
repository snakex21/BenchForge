...
const TAB_LABELS: Array<{ id: CompareTab; label: string }> = [{ id: 'table', label: 'Tabela' }, { id: 'bar', label: 'Słupkowy' }, { id: 'shape', label: 'Kształt' }, { id: 'ranking', label: 'Ranking' }]
...
export const CompareView: React.FC = () => {
  ...
  return <div className="space-y-6">
    <div className="flex items-center justify-between">
      <h2 className="text-xl font-bold text-slate-100">Porównaj modele</h2>
      <Button onClick={() => setSelectedModelIds(draftSelection)} disabled={draftSelection.length < 2}>Porównaj</Button>
    </div>

    <Card title="Wybierz modele do porównania" subtitle="Możesz zaznaczyć od 2 do 4 modeli.">
    ...
    {selectedModelIds.length < 2 ? <EmptyState icon="chart" title="Wybierz modele" description="Zaznacz minimum dwa modele i kliknij Porównaj." /> : !hasAnyResults ? <EmptyState icon="chart" title="Brak wyników dla wybranych modeli" description="Uruchom benchmarki w widoku Uruchom." actionLabel="Przejdź do Uruchom" onAction={() => setActiveView('runner')} /> : <Card title="Widok porównania">{activeTab === 'table' && renderTable()}{activeTab === 'bar' && renderBarChart()}{activeTab === 'shape' && renderRadar()}{activeTab === 'ranking' && renderRanking()}</Card>}
  </div>
}
