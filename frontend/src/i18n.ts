...
export type Language = 'pl' | 'en' | 'de' | 'es' | 'fr' | 'it' | 'pt' | 'uk' | 'cs' | 'nl' | 'tr' | 'ja' | 'ru' | 'zh' | 'zh-TW' | 'ko' | 'id' | 'vi' | 'th' | 'hi' | 'ar' | 'he' | 'el' | 'sv' | 'no' | 'da' | 'fi' | 'hu' | 'ro' | 'bg' | 'hr' | 'sk' | 'sl' | 'lt' | 'lv' | 'et' | 'sr' | 'fa' | 'ur' | 'ms' | 'fil' | 'bn' | 'ca' | 'eu' | 'gl' | 'ga' | 'cy' | 'is' | 'mt' | 'sq' | 'mk' | 'be' | 'bs' | 'lb' | 'gd' | 'br' | 'co' | 'fy'
...
export const LANGUAGE_OPTIONS: Array<{ id: Language; label: string; nativeLabel: string; flag: string; locale: string }> = [
  ...
  { id: 'ca', label: 'Catalan', nativeLabel: 'Català', flag: '🇦🇩', locale: 'ca-AD' },
  { id: 'eu', label: 'Basque', nativeLabel: 'Euskara', flag: '🇪🇸', locale: 'eu-ES' },
  { id: 'gl', label: 'Galician', nativeLabel: 'Galego', flag: '🇪🇸', locale: 'gl-ES' },
  { id: 'ga', label: 'Irish', nativeLabel: 'Gaeilge', flag: '🇮🇪', locale: 'ga-IE' },
  { id: 'cy', label: 'Welsh', nativeLabel: 'Cymraeg', flag: '🏴', locale: 'cy-GB' },
  { id: 'is', label: 'Icelandic', nativeLabel: 'Íslenska', flag: '🇮🇸', locale: 'is-IS' },
  { id: 'mt', label: 'Maltese', nativeLabel: 'Malti', flag: '🇲🇹', locale: 'mt-MT' },
  { id: 'sq', label: 'Albanian', nativeLabel: 'Shqip', flag: '🇦🇱', locale: 'sq-AL' },
  { id: 'mk', label: 'Macedonian', nativeLabel: 'Македонски', flag: '🇲🇰', locale: 'mk-MK' },
  { id: 'be', label: 'Belarusian', nativeLabel: 'Беларуская', flag: '🇧🇾', locale: 'be-BY' },
  { id: 'bs', label: 'Bosnian', nativeLabel: 'Bosanski', flag: '🇧🇦', locale: 'bs-BA' },
  { id: 'lb', label: 'Luxembourgish', nativeLabel: 'Lëtzebuergesch', flag: '🇱🇺', locale: 'lb-LU' },
  { id: 'gd', label: 'Scottish Gaelic', nativeLabel: 'Gàidhlig', flag: '🇬🇧', locale: 'gd-GB' },
  { id: 'br', label: 'Breton', nativeLabel: 'Brezhoneg', flag: '🇫🇷', locale: 'br-FR' },
  { id: 'co', label: 'Corsican', nativeLabel: 'Corsu', flag: '🇫🇷', locale: 'co-FR' },
  { id: 'fy', label: 'Frisian', nativeLabel: 'Frysk', flag: '🇳🇱', locale: 'fy-NL' },
...
const localeLoaders: Record<string, () => Promise<{ default: PartialDictionary }>> = {
  ...
  ca: () => import('./i18n/locales/ca.json'),
  eu: () => import('./i18n/locales/eu.json'),
  gl: () => import('./i18n/locales/gl.json'),
  ga: () => import('./i18n/locales/ga.json'),
  cy: () => import('./i18n/locales/cy.json'),
  is: () => import('./i18n/locales/is.json'),
  mt: () => import('./i18n/locales/mt.json'),
  sq: () => import('./i18n/locales/sq.json'),
  mk: () => import('./i18n/locales/mk.json'),
  be: () => import('./i18n/locales/be.json'),
  bs: () => import('./i18n/locales/bs.json'),
  lb: () => import('./i18n/locales/lb.json'),
  gd: () => import('./i18n/locales/gd.json'),
  br: () => import('./i18n/locales/br.json'),
  co: () => import('./i18n/locales/co.json'),
  fy: () => import('./i18n/locales/fy.json'),
